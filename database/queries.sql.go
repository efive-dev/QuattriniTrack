...
func (q *Queries) GetTransactionByCategoryID(ctx context.Context, categoriesID int64) ([]Transaction, error) {
	rows, err := q.db.QueryContext(ctx, getTransactionByCategoryID, categoriesID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
